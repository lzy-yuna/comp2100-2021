...
public class ThreeColourTree<T extends Comparable<T>> {
	...
	public Boolean testProp1Helper(Node<T> root) {
		if (root == null) {
			return true;
		} else {
			return root.colour != Colour.VIOLET && testProp1Helper(root.left) && testProp1Helper(root.right);
		}
	}
	...
	public Boolean testProp2() {
		//START YOUR CODE
		return testProp2Helper(this.root);
		//END YOUR CODE
	}

	public boolean testProp2Helper(Node<T> root) {
		if (root.parent == null) {
			return root.colour == Colour.PINK && testProp2Helper(root.left) && testProp2Helper(root.right);
		} else if (root.value == null) {
			return root.colour == Colour.PINK;
		} else {
			return true;
		}
	}
	...
	public Boolean testProp3() {
		//START YOUR CODE
		Set<Integer> pink_nums = new HashSet<>();
		testProp3Helper(this.root, pink_nums, 0);
		return pink_nums.size() <= 1;
		//END YOUR CODE
	}

	public void testProp3Helper(Node<T> root, Set<Integer> pink_nums, int current_count) {
		if (root.value == null) {
			current_count++;
			pink_nums.add(current_count);
		} else {
			if (root.colour == Colour.PINK)
				current_count++;
			testProp3Helper(root.left, pink_nums, current_count);
			testProp3Helper(root.right, pink_nums, current_count);
		}
	}
	...
	public Boolean testProp4() {
		//START YOUR CODE
		return testProp4Helper(this.root);
		//END YOUR CODE
	}

	public boolean testProp4Helper(Node<T> root) {
		if (root.colour == Colour.MAGENTA) {
			boolean current = (root.left.colour != Colour.MAGENTA && root.right.colour != Colour.MAGENTA) &&
					(root.left.colour == Colour.PURPLE || root.right.colour == Colour.PURPLE);
			return current && testProp4Helper(root.left) && testProp4Helper(root.right);
		} else {
			return true;
		}
	}
