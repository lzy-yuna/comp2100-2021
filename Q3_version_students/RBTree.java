...
public class RBTree<T extends Comparable<T>> {
	...
	public boolean prop1(Node<T> root) {
		if (root.parent == null) {
			return root.colour == Colour.BLACK && prop1(root.left) && prop1(root.right);
		} else {
			if (root.value == null) {
				return root.colour == Colour.BLACK;
			}
			return prop1(root.left) && prop1(root.right);
		}
	}
	...
	public boolean prop2(Node<T> root) {
		if (root.value != null) {
			...
			return prop2(root.right) && prop2(root.left);
		}
		return true;
	}
